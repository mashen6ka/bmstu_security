...
(это вот то, что находится внутри цикла по 16 раундовым ключам)
...
2. `Temp = xor(Temp, текущий ключ)`. Для шифрования $i$й ключ, для расшифровки - $(15 - i)$й.
...
> **Каким условиям должна удовлетворять открытая экспонента? Секретная?**

Открытая экспонента - это `e`, секретная экспонента - это `d`.

Открытая экспонента должна быть взаимно простой с `phi(n)` (больше 1, но меньше `phi(n)`).

Секретная экспонента должна быть взаимно обратной с `e` по модулю `phi(n)`.
...
Yes. Причем в зависимости от заданной длины ключа может очень сильно отличаться. Мы по факту каждый байт исходного текста переводим в какое-то числовое значение, которое может представляться несколькими байтами.
...
(Я не юзала, вроде питоновская функция шустрее)

```
def __powLogN(x: int, y: int, p: int) -> int:
...
> **Какие проблемы могут быть при записи?**

Нецелое число байт данных для записи (см. вопрос про лишние биты).
